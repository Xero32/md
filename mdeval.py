...
for ctr, jb in enumerate(jobs):
    for d in range(1, NUM_OF_TRAJ+1):
        ...
        for i in range(0,nb):
            flag = bounce.Population(i, Bounce[:,i,j], Bounce[:,i+1,j], State, Trans, Refl, SCALING, j)
            if flag == -6000 or flag == -5000:
                TOTAL[int(Bounce[1,i,j]):TIMESTEPS] -= 1
                print(str(flag))
                break
            elif flag == -3000:
                break
...
StickCoeff = stats.InitSticking(Refl, float(Angle[angle]),float(Temperature[temperature]), float(Energy[energy]), NUM_OF_JOBS*NUM_OF_TRAJ)

plot.Populations(angle, temperature, Energy[energy], Time, State[0], State[1], State[2], smflag=0, pltflag=0)
plot.TransitionPopulations(angle, temperature, Energy[energy], Time, Trans[0], Trans[2], Trans[1], Trans[3], Trans[4], Trans[5], smflag=0, pltflag=1)
#plot.TransitionPopulations(angle, temperature, Energy[energy], Time, StateNew[0], State[0], StateNew[1], State[1], smflag=0, pltflag=1)

'''
for i in range(0, TIMESTEPS):
    Sum1 = StateNew[0,i] + StateNew[1,i] + StateNew[2,i]
    Sum2 = State[0,i] + State[1,i] + State[2,i]
    if Sum1 != 1:
        #print(str(i), "Sum1: ", str(Sum1))
    if Sum2 != 1:  # large deviations!
        #print(str(i), "Sum2: ", str(Sum2))
'''
Sum1 = StateSum[0,-10] + StateSum[1,-10] + StateSum[2,-10]
Sum2 = State[0,-10] + State[1,-10] + State[2,-10]
...
#for k in range(0,4):
#    TransNew[k] = bounce.Integrate(TransNew[k], Trans[k], 1)

#plot.TransitionPopulations(angle, temperature, Energy[energy], Time, TransNew[0], TransNew[2], TransNew[1], TransNew[3], smflag=0, pltflag=1)
'''State = np.zeros([3,TIMESTEPS]) # T, Q, C
...
print(State[0,-1])
print(State[1,-1])
left = State[0,-1] * TRate[0,-1]
right = State[1,-1] * TRate[2,-1] + State[1,-1] * TRate[3,-1]
...
'''
TransNew[2] = bounce.IntegrateTransition(TransNew[2], TRate[2], State[1], 1)
TransNew[0] = bounce.IntegrateTransition(TransNew[0], TRate[0], State[0], 1)
TransNew[1] = bounce.IntegrateTransition(TransNew[1], TRate[1], State[0], 1)
#TransNew[2] = bounce.IntegrateTransition(TransNew[2], TRate[2], State[1], 1)
TransNew[3] = bounce.IntegrateTransition(TransNew[3], TRate[3], State[1], 1)
#plot.TransitionPopulations(angle, temperature, Energy[energy], Time, TransNew[0], TransNew[2], TransNew[1], TransNew[3], smflag=0, pltflag=0)
'''
'''
for n in range(0,30):
    #sprint(str(n))
